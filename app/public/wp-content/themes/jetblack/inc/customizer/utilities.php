...
class JetBlack_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'jetblack_customizer_defaults', array() );
	...
	public static function get_posts_as_array( $args ) {
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					$title = sprintf( esc_html__( 'ID: %s No Title', 'jetblack' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			...
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'jetblack' ),
			'entire-site'  => esc_html__( 'Entire Site', 'jetblack' ),
		);

		return apply_filters( 'jetblack_section_visibility_options', $options );
	}
}
