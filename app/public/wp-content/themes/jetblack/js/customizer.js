...
jQuery( document ).ready(function($) {
	 ...
	 */
	// Init sortable.
	$('#sub-accordion-panel-jetblack_sp_sortable').sortable( {
		items : "> li:gt(0)",
		axis : 'y',
		helper : 'clone',
		// Update value when we stop sorting.
		update: function( event, ui ) {
			var inputValues = $(this).sortable( "toArray" );

			// Remove prefix.
			$.each( inputValues, function( index, value ) {
				inputValues[index] = inputValues[index].replace('accordion-section-jetblack_ss_','');
			});

			inputValues = inputValues.filter( onlyUnique );
			
			inputValues = inputValues.join( ',' );

			// Add the value to the hidden field
			$( '#_customize-input-jetblack_ss_order' ).val( inputValues ).trigger('change');
